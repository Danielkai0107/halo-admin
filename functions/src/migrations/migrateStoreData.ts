 ...
 * 3. 檢查回傳結果確認遷移成功
 */
export const migrateStoreData = onRequest(async (req, res) => {
  // CORS handling
 ...
 * 僅在需要回退時使用
 */
export const rollbackStoreMigration = onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  res.set("Access-Control-Allow-Methods", "GET, OPTIONS");
  res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
  ...
  try {
    ...
    for (const gatewayDoc of gatewaysSnapshot.docs) {
      ...
      if (!storeDoc.exists) {
        console.warn(
          `Store ${storeId} not found, skipping gateway ${gatewayId}`,
        );
        continue;
      }
