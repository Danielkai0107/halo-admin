  ...
  try {
    ...
    for (const tenantDoc of tenantsSnap.docs) {
      ...
      if (tenantData.lineChannelSecret && signature) {
        try {
          if (
            (0, bot_sdk_1.validateSignature)(
              body,
              tenantData.lineChannelSecret,
              signature,
            )
          ) {
            matchedTenant = {
          ...
          // 繼續嘗試下一個社區
          console.warn(
            `Failed to validate signature for tenant ${tenantDoc.id}:`,
            e,
          );
        }
      }
...
async function handleFollow(event, matchedTenant) {
  ...
  try {
    ...
    const channelAccessToken =
      (matchedTenant === null || matchedTenant === void 0
        ? void 0
        : matchedTenant.channelAccessToken) || config.channelAccessToken;
      ...
      try {
        const client = new bot_sdk_1.Client({
          ...
          channelSecret:
            (matchedTenant === null || matchedTenant === void 0
              ? void 0
              : matchedTenant.channelSecret) || config.channelSecret,
        });
        ...
        console.warn(
          "Failed to get LINE profile, using defaults:",
          profileError,
        );
    ...
    if (!existingUserQuery.empty) {
      ...
      console.log(
        ...
        lineUserId,
        lineDisplayName,
        matchedTenant ? `from tenant ${matchedTenant.id}` : "",
      );
    }
...
async function handlePostback(event, matchedTenant) {
  ...
  try {
    ...
    const tenantId =
      alertData === null || alertData === void 0 ? void 0 : alertData.tenantId;
    // 確定使用的 Channel Access Token
    const channelAccessToken =
      (matchedTenant === null || matchedTenant === void 0
        ? void 0
    ...
    // 檢查是否為被分配者
    if (
      (alertData === null || alertData === void 0
        ? void 0
        : alertData.assignedTo) !== appUserId
    ) {
      ...
      if (channelAccessToken) {
        await (0, sendMessage_1.sendNotification)(
          lineUserId,
          channelAccessToken,
          "⚠️ 此警報未分配給您，無法操作。",
        );
    ...
    if (
      (alertData === null || alertData === void 0
        ? void 0
        : alertData.assignmentStatus) !== "PENDING"
    ) {
...
async function notifyAdmins(db, tenantId, channelAccessToken, data) {
  try {
    ...
    const notifications = adminsQuery.docs.map(async (doc) => {
      ...
      if (
        (adminUser === null || adminUser === void 0
          ? void 0
