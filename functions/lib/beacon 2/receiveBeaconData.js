...
async function sendLineNotificationToTenant(elderId, elder, beacon, gateway, lat, lng, timestamp, tenantId, channelAccessToken, db) {
    try {
        ...
        if (gateway.type === 'OBSERVE_ZONE' || gateway.type === 'INACTIVE') {
            console.log(`Skipping notification for ${gateway.type} gateway (notification disabled for this type)`);
            return;
        }
        ...
        if (!isFirstActivity && !notificationEnabled) {
        ...
        const gatewayTypeText = gateway.type === 'SCHOOL_ZONE' ? '學區' :
            gateway.type === 'SAFE_ZONE' ? '安全區' :
                gateway.type === 'OBSERVE_ZONE' ? '觀察區' : '停用';
        const headerText = '位置更新';
        const bodyText = `${(elder === null || elder === void 0 ? void 0 : elder.name) || '長輩'} 位置已更新`;
...
// Note: Boundary alert function removed - gateway types changed to SCHOOL_ZONE, SAFE_ZONE, OBSERVE_ZONE, INACTIVE
/**
