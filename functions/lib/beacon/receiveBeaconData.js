...
async function handleLineUserNotification(deviceId, lineUserDocId, beacon, gateway, lat, lng, timestamp, db) {
    try {
        // Skip notifications for OBSERVE_ZONE and INACTIVE gateways
        if (gateway.type === "OBSERVE_ZONE" || gateway.type === "INACTIVE") {
            console.log(`Skipping LINE notification for ${gateway.type} gateway (notification disabled for this type)`);
            return { triggered: false, type: null };
        }
        // Get device data
        const deviceDoc = await db.collection("devices").doc(deviceId).get();
        ...
        if (!notificationPointIds.includes(gateway.id)) {
            console.log(`Gateway ${gateway.id} is not in LINE user's notification points`);
            return { triggered: false, type: null };
        }
        console.log(`Gateway ${gateway.id} is in LINE user's notification points, triggering notification`);
        // Get LINE user data
        ...
        const lineUserId = lineUserData === null || lineUserData === void 0 ? void 0 : lineUserData.lineUserId;
        if (!lineUserId) {
            console.log(`LINE user ${lineUserDocId} has no lineUserId, cannot send notification`);
            return { triggered: false, type: null };
        }
        ...
        try {
            ...
            const lineUserDoc = await db
                ...
                .get();
            const lineUserData = lineUserDoc.data();
            const joinedFromTenantId = lineUserData === null || lineUserData === void 0 ? void 0 : lineUserData.joinedFromTenantId;
            if (!joinedFromTenantId) {
                console.warn(`LINE user ${lineUserId} has no joinedFromTenantId, checking memberships...`);
                ...
                if (!foundTenantId) {
                    console.warn(`LINE user ${lineUserId} has no associated tenant, skipping LINE notification`);
                    return {
                        triggered: true,
                        type: "LINE",
                        pointId: gateway.id,
                ...
                const lineChannelAccessToken = tenantData === null || tenantData === void 0 ? void 0 : tenantData.lineChannelAccessToken;
                if (!lineChannelAccessToken) {
                    console.warn(`Tenant ${foundTenantId} has no LINE Channel Access Token`);
                }
                else {
                    ...
                    console.log(`Sent LINE notification to ${lineUserId} for gateway ${gateway.name}`);
                }
            }
            else {
                // Get tenant's LINE Channel Access Token
                const tenantDoc = await db
                    ...
                    .get();
                if (!tenantDoc.exists) {
                    console.warn(`Tenant ${joinedFromTenantId} not found, skipping LINE notification`);
                }
                else {
                    const tenantData = tenantDoc.data();
                    const lineChannelAccessToken = tenantData === null || tenantData === void 0 ? void 0 : tenantData.lineChannelAccessToken;
                    if (!lineChannelAccessToken) {
                        console.warn(`Tenant ${joinedFromTenantId} has no LINE Channel Access Token`);
                    }
                    else {
                        // Import sendNotificationPointAlert function
                        const { sendNotificationPointAlert } = await Promise.resolve().then(() => __importStar(require("../line/sendMessage")));
                        ...
                        console.log(`Sent LINE notification to ${lineUserId} for gateway ${gateway.name} using tenant ${joinedFromTenantId}`);
                    }
                }
            }
        }
        ...
        console.error(`Error in handleLineUserNotification for user ${lineUserDocId}:`, error);
        return { triggered: false, type: null };
    }
}
/**
