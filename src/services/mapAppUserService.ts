...
export const mapAppUserService = {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const notificationEnabled =
          (mapAppUser as any).notificationEnabled !== false;
        await updateDocument("app_users", id, {
  ...
  bindDevice: (
    ...
    deviceOwnerAge?: number,
    deviceOwnerGender?: "MALE" | "FEMALE" | "OTHER",
  ) => {
    return new Promise(async (resolve, reject) => {
      try {
        // 獲取當前用戶的 ID Token
  ...
  // 刪除Line 用戶管理（調用 Cloud Function 進行完整刪除）
  delete: (id: string) => {
    return new Promise(async (resolve, reject) => {
      try {
        // 獲取當前用戶的 ID Token
        ...
        const response = await fetch(
          "https://us-central1-safe-net-tw.cloudfunctions.net/deleteMapAppUser",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
