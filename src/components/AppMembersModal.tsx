...
export const AppMembersModal = ({ isOpen, onClose, tenantId, tenantName }: AppMembersModalProps) => {
  ...
  const handleToggleRole = async (memberId: string, currentRole: string, memberName: string, appUserId: string) => {
  ...
  const handleRemoveMember = async (memberId: string, memberName: string, appUserId: string) => {
    ...
    try {
      ...
      if (memberId.startsWith('virtual_')) {
        ...
        await updateDocument('appUsers', appUserId, {
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3 flex-1">
                      ...
                      {member.appUser?.linePictureUrl ? (
                        <img 
                          ...
                          alt={member.appUser?.lineDisplayName || member.appUser?.name}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2 flex-wrap">
                          ...
                          {member.appUser?.lineDisplayName && member.appUser.lineDisplayName !== member.appUser.name && (
                        ...
                        <div className="text-sm text-gray-600 space-y-1">
                          ...
                          {member.createdAt && (
                            <div>
                              加入時間: {member.createdAt.toDate ? new Date(member.createdAt.toDate()).toLocaleString('zh-TW') : new Date(member.createdAt).toLocaleString('zh-TW')}
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center space-x-2">
                      {/* 顯示管理員切換按鈕 */}
                      <button
                        onClick={() => handleToggleRole(member.id, member.role || 'MEMBER', member.appUser?.name || '', member.appUserId)}
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium ${
                          (member.role || 'MEMBER') === 'ADMIN'
                            ? 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                            : 'bg-blue-100 text-blue-700 hover:bg-blue-200'
                        }`}
                        title={(member.role || 'MEMBER') === 'ADMIN' ? '取消管理員' : '設為管理員'}
                      >
                        {(member.role || 'MEMBER') === 'ADMIN' ? '取消管理員' : '設為管理員'}
                      </button>
                      <button
                        onClick={() => handleRemoveMember(member.id, member.appUser?.name || '', member.appUserId)}
