...
export const notificationPointService = {
  ...
  subscribe: (
    tenantId: string,
    callback: (data: TenantNotificationPoint[]) => void,
  ) => {
      ...
      async (points) => {
        ...
        const pointsWithGateway = await Promise.all(
          points.map(async (point) => {
            if (point.gatewayId) {
              const gateway = await getDoc(
                doc(db, "gateways", point.gatewayId),
              );
              if (gateway.exists()) {
                return {
                  ...point,
  ...
  // 獲取所有可用的 gateways（不限社區，讓Line OA 管理員自行選擇）
  ...
  syncTenantDevices: async (tenantId: string) => {
    try {
      ...
      console.log(
        `Found ${gatewayIds.length} active notification points for tenant ${tenantId}`,
      );
      ...
      console.log(
        `Found ${devicesSnapshot.docs.length} devices in tenant ${tenantId}`,
      );
      ...
      const updatePromises = devicesSnapshot.docs.map((deviceDoc) => {
        const updateData: any = {
          inheritedNotificationPointIds:
            gatewayIds.length > 0 ? gatewayIds : null,
        };
      ...
      console.log(
        `Successfully synced ${devicesSnapshot.docs.length} devices with ${gatewayIds.length} notification points`,
      );
