...
- ⚠️ 如果設備已被地圖用戶綁定，改為 `TENANT` 前請先在「Line 用戶管理管理」中解綁
...
| 池類型 | 顯示             | 說明                       |
| ------ | ---------------- | -------------------------- |
| PUBLIC | 🌍 PUBLIC (綠色) | 可被Line 用戶管理綁定      |
...
1. 在「Line 用戶管理管理」中確認設備未被綁定
2. 編輯設備，將 poolType 改為 `TENANT`
3. 可在「Line OA 管理」中分配給社區
4. 在「長者管理」中分配給長者
